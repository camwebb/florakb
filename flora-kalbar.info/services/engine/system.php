...
if (is_array($CONFIG)) {
	...
	if ($CONFIG['services']['app_underdevelopment'] == TRUE) under_development();
...
if (isset($validation)) {
	...
	if (isset($validation['pid'])) {
		...
		if ($validation['pid'] == ''){
			...
			$setPage = $CONFIG['services']['default_view'];
...
// pr($validation);exit;
$DATA['services']['page'] = @$setPage ;
$DATA['services']['function'] = @$setFunction ;
$DATA['services']['uri'] = @$validation ;
